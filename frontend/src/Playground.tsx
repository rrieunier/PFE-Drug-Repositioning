...
class Playground extends React.Component<Props, AppState> {
  // Maps to store key -> arr index for quick lookups
  private mapNodeKeyIdx: Map<go.Key, number>;
  private mapLinkKeyIdx: Map<go.Key, number>;

  private initGraph = (data: GraphJSON) => {
  ...
  shouldComponentUpdate(
    ...
    console.log(props.nodeDataArray, state.nodeDataArray);
    this.refreshNodeIndex(this.state.nodeDataArray);
    this.refreshLinkIndex(this.state.linkDataArray);
    // if (props.nodeDataArray.length === state.nodeDataArray.length) {
    // if ( props.nodeDataArray.length > 0 && state.nodeDataArray.length > 0 ) {
    //   if( props.nodeDataArray[0].text !== state.nodeDataArray[0].text )
    // }
    // } else
    //   return this.initGraph({
    //     nodes: props.nodeDataArray,
    //     links: props.linkDataArray
    //   });
    // return this.initGraph({
    //   nodes: props.nodeDataArray,
    //   links: props.linkDataArray
